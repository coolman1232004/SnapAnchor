<p align="center">
  <img src="Assets/SnapPin-icon-512.png" width="104" alt="SnapPin icon">
</p>

<h1 align="center">SnapPin</h1>
...
<p align="center">
  ...
  <img alt="Current version" src="https://img.shields.io/badge/version-1.2.14-2C978E">
...
SnapPin keeps the whole capture workflow close to the screen: select a region, annotate it in place, copy or save it, pin it above other windows, extract text locally, or record the selected area. No account is required and recognition runs on the computer.

> [!IMPORTANT]
> SnapPin is an independent project. It does not contain Snipaste code, branding, icons, assets, or reverse-engineered internals, and it is not affiliated with or endorsed by Snipaste.
...
- Privacy exclusions for SnapPin and selected applications during captures and recordings
...
Use the [latest GitHub release](https://github.com/coolman1232004/SnapPin/releases/latest) for ready-to-run builds. Do not download the automatically generated source-code archive unless you intend to compile SnapPin yourself.

### Windows installer

1. Download `SnapPin-Setup-win-x64.exe` from the latest release.
2. Open it and follow the setup wizard.
3. Choose the install location and optional Desktop/Start Menu shortcuts.
4. Leave **Launch SnapPin** selected on the final page if you want to start immediately.

The installer places SnapPin under `%LOCALAPPDATA%\Programs\SnapPin` by default and registers a normal Windows uninstaller.
...
|---|---|
| ![SnapPin setup welcome](packaging/published-setup-previews/setup-step-1.png) | ![SnapPin setup options](packaging/published-setup-previews/setup-step-2.png) |

| Installation progress | Finish and launch |
|---|---|
| ![SnapPin setup progress](packaging/published-setup-previews/setup-step-3.png) | ![SnapPin setup complete](packaging/published-setup-previews/setup-step-4.png) |

</details>

### Portable version

1. Download `SnapPin-Portable-win-x64.zip` from the latest release.
...
```powershell
Get-FileHash .\SnapPin-Setup-win-x64.exe -Algorithm SHA256
```
...
SnapPin is designed to work locally:
...
- There is no SnapPin account, telemetry service, or cloud storage dependency.
...
Review the code and third-party components before using SnapPin in a sensitive or regulated environment.
...
```powershell
dotnet restore .\SnapPin.csproj
dotnet build .\SnapPin.csproj -c Release -p:Platform=x64
dotnet run --project .\SnapPin.csproj -c Release -p:Platform=x64
```
...
```powershell
.\packaging\Build-Packages.ps1 -Version 1.2.14
```
...
```powershell
SnapPin.exe capture --delay 3 --size 1280x720 --ratio 16:9 --cursor
SnapPin.exe capture-copy
SnapPin.exe pin
SnapPin.exe record
SnapPin.exe history
SnapPin.exe whiteboard
SnapPin.exe transparent-whiteboard
```
...
SnapPin is a substantial working application, but it is still an independently developed project. Before treating a release as production-ready, test the features you rely on across your monitor scaling, Windows version, audio devices, and security software.
...
- Use a **bug report** for reproducible problems and include the SnapPin version, Windows version, display scaling, and exact steps.
...
SnapPin is open-source software released under the [MIT License](LICENSE). Contributions are welcome when they follow [CONTRIBUTING.md](CONTRIBUTING.md).

## Third-party software

SnapPin uses Tesseract, ZXing.Net, ScreenRecorderLib, SkiaSharp, and their transitive components. Their licenses and notices remain the property of their respective authors. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
